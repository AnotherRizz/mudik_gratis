...
<body class=" bg-slate-200">
    <main class=" w-full h-screen flex justify-center align-middle">
        ...
        <div class=" w-full max-w-4xl h-2/3 mt-16 p-6 bg-white">
         <div class=" grid grid-cols-3">
            <div>
               <img src="{{ asset('images/logo-mudik.png') }}" class=" w-full" alt="">
            </div>
            <div class=" col-span-2 ">
               <h1 class=" font-extrabold text-slate-500 text-center text-2xl">REGISTER</h1>
               <form action="{{ route('register') }}" method="POST">
                  @csrf
                  <div class=" w-full grid grid-cols-2 gap-4  mt-16">
                      <div class="mb-4">
                        <label for="name" class="block  text-slate-500 font-bold text-sm">username</label>
                          <input type="text" id="name" name="name"  placeholder="username"
                          class="w-full px-3 py-2 border-2 rounded-sm shadow-sm text-slate-500 text-sm font-semibold focus:outline-none focus:ring-2 focus:ring-sky-500 focus:border-transparent @error('name') border-red-500 @enderror">
                          @error('name')
                      ...
                      <!-- Konfirmasi Password -->
                      <div class="mb-4">
                          <label for="password_confirmation" class="block  text-slate-500 font-bold text-sm">ulangi
                              Password</label>
                          <input type="password" id="password_confirmation" name="password_confirmation"
                              class="w-full px-3 py-2 border-2 rounded-sm shadow-sm text-slate-500 text-sm font-semibold focus:outline-none focus:ring-2 focus:ring-sky-500 focus:border-transparent">
                      </div>
                  </div>
                  <div class="mt-5">
                     <a href=""><button class=" px-3 py-1 font-bold bg-sky-500 rounded-md text-white w-full ">create account
                     </button>
                  </a>
